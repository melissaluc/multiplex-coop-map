...
// From form input
type UserUnmappableInputs = {
  SITE_PARKING: boolean;
  NUMBER_OF_UNITS?: number;
  BUILDING_FOOTPRINT_AREA?: number;
};

// Fields Mappable to Dataset
type UserMappableInputs = {
  HT_STORIES?: number;
  // HT_STRING?: string; // has stories & height in string
  HT_LABEL?: number; //
  FSI_TOTAL?: number; // floor_area/ lot_area or building_footprint * stories / lot_area
  COVERAGE?: number; // LOT_COVERSGE
  PRCNT_RES?: number;
  ZN_AREA?: number; //LOT_AREA
  AREA_UNITS?: string; // MIN_DWELL_AREA_UNIT?
  STATEDAREA?: number;
  GEN_ZONE?: number;
  FRONTAGE?: number;
};

// Combined type for all user inputs
type UserInputs = UserUnmappableInputs & UserMappableInputs;

const connection = await createDuckDB();

// calculate necessary fields from user inputs
function calculateDerivedFields(input: UserInputs): void {
  // denisty_group: fsi_total or [number_of_units, minumum_unit_size]
  if (input.AREA_UNITS && input.NUMBER_OF_UNITS) {
    if (input.BUILDING_FOOTPRINT_AREA) {
      // FSI_TOTAL = input.BUILDING_FOOTPRINT_AREA*HT_STORIES/STATEDAREA
    } else {
      // FSI_TOTAL = input.MIN_DWELL_AREA_UNIT*input.NUMBER_OF_UNITS/STATEDAREA
    }
    // building_area: [building_footprint_area, lot_coverage_percent]
    if (input.BUILDING_FOOTPRINT_AREA) {
      // Need lot area: COVERAGE or PRCNT_CVER = STATEDAREA/input.BUILDING_FOOTPRINT_AREA
    }

    if (input.COVERAGE) {
      // BUILDING_FOOTPRINT_AREA = STATEDAREA*input.LOT_COVERAGE
    }
    // building_height: [ht_height, ht_stories] => HT_STRING
  }
}

function validateUserInputs(input: UserInputs): void {
  const errors: string[] = [];

  // Height: one of stories or height string
  if (input.HT_STORIES == null && input.HT_STRING == null) {
    errors.push(
      "Please provide either building stories (HT_STORIES) or building height (HT_STRING)."
    );
  }

  // Density: either unit count or FSI
  if (input.FSI_TOTAL == null) {
    errors.push(
      "Please provide either DENSITY (unit count) or FSI_TOTAL (Floor Space Index)."
    );
  }

  // Coverage: either lot coverage % or reserved percentage
  if (input.COVERAGE == null && input.PRCNT_RES == null) {
    errors.push(
      "Please provide either COVERAGE (lot coverage %) or PRCNT_RES (percent reserved)."
    );
  }

  // Lot area: area or area units
  if (input.ZN_AREA == null && input.AREA_UNITS == null) {
    errors.push("Please provide either ZN_AREA or AREA_UNITS.");
  }

  // Frontage: either width or stated area
  if (input.FRONTAGE == null) {
    errors.push("Please provide either FRONTAGE");
  }

  // Required: zone
  if (input.GEN_ZONE == null) {
    errors.push("GEN_ZONE (Zone Type) is required.");
  }
...
// Query for height/stories requirement
async function getFilteredZoningHeightOverlay() {
  const filePath = getFilePath(ZoningHeightOverlay.name);
  const result = await connection.stream(`
      SELECT _id, HT_STORIES, HT_LABEL, geometry
